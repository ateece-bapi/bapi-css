...
add_filter('woocommerce_my_account_my_orders_columns', function($columns) {
	$columns['po-num'] = __('Customer PO #', 'woocommerce');
	$columns['ba-num'] = __('BAPI Sales Order #', 'woocommerce');
	$columns['track-num'] = __('Tracking #', 'woocommerce');
	return $columns;
});
		...
		<tbody>
			...
			foreach ( $customer_orders->orders as $customer_order ) {
				$order      = wc_get_order( $customer_order ); // phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited
				   ...
				   $track_id   = $order->get_tracking_id();
				?>
				<tr class="woocommerce-orders-table__row woocommerce-orders-table__row--status-<?php echo esc_attr( $order->get_status() ); ?> order">
					...
					?>
						...
						<?php endif; ?>
							       ...
							       <?php elseif ( 'po-num' === $column_id ) : ?>
								       <?php echo $order->get_po_number(); ?>

							       <?php elseif ( 'ba-num' === $column_id ) : ?>
								       <?php echo $order->get_bapi_order_number(); ?>

							       <?php elseif ( 'track-num' === $column_id ) : ?>
								       <?php if ( $track_id ) { ?>
									       <a title="Track this order at UPS.com (opens new tab/window)" target="_blank" href="http://wwwapps.ups.com/etracking/tracking.cgi?TypeOfInquiryNumber=T&amp;InquiryNumber1=<?php echo $track_id; ?>">Track order</a>
								       <?php } ?>
